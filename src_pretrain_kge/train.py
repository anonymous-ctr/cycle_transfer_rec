...
def train(args, data, show_loss, show_topk):
    ...
    with tf.Session() as sess:
        ...
        for step in range(args.n_epochs):
            ...
            # CTR evaluation
            # train_auc, train_acc, _, _, _ = model.eval(sess, get_feed_dict_for_rs(model, train_data, 0, train_data.shape[0]))
            # eval_auc, eval_acc, _, _, _ = model.eval(sess, get_feed_dict_for_rs(model, eval_data, 0, eval_data.shape[0]))
            # test_auc, test_acc, test_pr, test_re, test_f1 = model.eval(sess, get_feed_dict_for_rs(model, test_data, 0, test_data.shape[0]))

            # print('epoch %d    train auc: %.4f  acc: %.4f    eval auc: %.4f  acc: %.4f    test auc: %.4f  acc: %.4f, pre: %.4f, rec: %.4f, f1: %.4f'
            #       % (step, train_auc, train_acc, eval_auc, eval_acc, test_auc, test_acc, test_pr, test_re, test_f1))

            if args.ndcg:
                ndcg_results = model.calc_ndcg(sess, model, train_data, test_data, 100 * args.batch_size)
                print('\t', ndcg_results)
...
def get_feed_dict_for_kge(model, kg, start, end):
    feed_dict = {model.head_indices: kg[start:end, 0],
